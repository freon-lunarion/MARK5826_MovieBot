...
def title_opt(search_term, channel):
...
def handle_command(message, channel, message_user,context):
  ...
  try:
    if response['context']['movie_name'] not in ('1','2','3','4','5'):
  ...
  try:
    ...
    if search_term.strip() != '' and 'option' in response['context'] and str(response['context']['option']) in ['1','2','3','4','5']:

      selection = int(response['context']['option']) - 1
      b = similarity_search(search_term, list(metadata.index))
      response['context']['movie_name'] = metadata.loc[b]['title'].values[selection]
      try:
        resp = service.create_synonym(
          workspace_id = workspace_id,
          entity = 'movie_name',
          value = response['context']['movie_name'],
          synonym= search_term.strip()
        ).get_result()

      except :
        resp = service.create_value(
          workspace_id = workspace_id,
          entity = 'movie_name',
          value = response['context']['movie_name'],
  ...
  try:
    ...
    # search_term = message.split("to",1)[1]
    pass
